...
for (const ref of loadCorpus('gtzan').filter((_, i) => i % shards === shard)) {
	...
	// The phase that actually maximises downbeat F, from the annotation.
	...
	if (fAt(truePhase) < 0.5) continue;   // beats too poor for the phase question to be meaningful
	...
	const cues: Record<string, Float32Array> = {
		harmonic: z(harmonic), bassJump: z(bassJump), flux: z(bf.flux),
		low: z(bf.low), mid: z(bf.mid), high: z(bf.high), rms: z(bf.rms), bass: z(bf.bass)
	};
	const sep = (c: Float32Array, p: number) => {
		let on = 0, onN = 0, off = 0, offN = 0;
		for (let i = 0; i < n; i++) { if (i % bpb === p) { on += c[i]; onN++; } else { off += c[i]; offN++; } }
		return on / Math.max(1, onN) - off / Math.max(1, offN);
	};
	const pick = (c: Float32Array, sign = 1) => { let best = 0; for (let p = 1; p < bpb; p++) if (sign * sep(c, p) > sign * sep(c, best)) best = p; return best; };

	const out: Record<string, number> = { truePhase, detected: m.phase };
	for (const [k, c] of Object.entries(cues)) out[k] = pick(c);
	out.midNeg = pick(cues.mid, -1);
	// The current combination, restated.
	const combined = new Float32Array(n);
	for (let i = 0; i < n; i++) combined[i] = 1.2 * cues.harmonic[i] + 0.25 * cues.bassJump[i] + 0.5 * cues.flux[i] + 0.8 * cues.low[i] - 0.8 * cues.mid[i];
	out.combined = pick(combined);
	console.log(JSON.stringify({ key: ref.key, ...out }));
}
