...
/// at the room. `leds` is read from the output module rather than written here, so the day the
/// strips are actually driven the answer changes with them.
